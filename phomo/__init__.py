...
def generate_random_corpus(k=2, num_letters=10, num_restrictions=5, max_word_length=10, corpus_length=1000):
    ...
    def make_word(max_length, symbols, allowed_dictionary):
        ...
        # Converts the list to a string (word)
        word = ''.join([item for t in word for item in t])
        
        return word
    
    def make_corpus(max_word_length, corpus_length, symbols, allowed_dictionary):
        return " ".join([make_word(max_word_length, symbols, allowed_dictionary) for i in range(corpus_length)])

    # TODO: Update letters with IPA symbols. 
    letters = list(string.ascii_lowercase)
    symbols = set([random.choice(letters) for i in range(num_letters)])
